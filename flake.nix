  ...
  outputs = { self, nixpkgs, flake-utils, pip2nix, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        packageOverrides =
          nixpkgs.lib.composeManyExtensions [ pythonOverrides customOverrides ];
      ...
      in {
        packages = {
          ...
          inventree-server = with import nixpkgs { inherit system; };
            pkgs.writeShellApplication rec {
              ...
              runtimeInputs =
                [ pythonWithPackages self.packages.${system}.inventree-src ];
          ...
          inventree-refresh-users = with import nixpkgs { inherit system; };
            let
              refreshScript = pkgs.writeScript "refresh_users.py"
                (builtins.readFile ./refresh_users.py);
            in pkgs.writeShellApplication rec {
          ...
          default = pkgs.symlinkJoin {
            name = "default inventree packges";
            paths = [
        ...
        nixosModule = { lib, pkgs, config, ... }:
          ...
          let
            ...
            configFile =
              pkgs.writeText "config.yaml" (builtins.toJSON cfg.config);
          ...
          in {
            options.services.inventree = {
              ...
              secretPath = mkOption { type = types.path; };
            ...
            config = mkIf cfg.enable ({
                ...
                serviceConfig = {
                  ...
                  ExecStartPre = "+${
                      pkgs.writers.writeBash "inventree-setup" ''
                        ...
                        ${
                          self.packages.${pkgs.system}.inventree-invoke
                        }/bin/inventree-invoke migrate

                        echo "Ensuring static files are populated"
                        ${
                          self.packages.${pkgs.system}.inventree-invoke
                        }/bin/inventree-invoke static

                        echo "Setting up users"
                        cat ${usersFile} | \
                          ${
                            self.packages.${pkgs.system}.inventree-refresh-users
                          }/bin/inventree-refresh-users
                      ''
                    }";
                  ExecStart = ''
                    ${
                      self.packages.${pkgs.system}.inventree-server
                    }/bin/inventree-server -b ${cfg.serverBind}
                  '';
                };
              };
              systemd.services.inventree-cluster = {
                ...
                serviceConfig = {
                  ...
                  ExecStart = ''
                    ${
                      self.packages.${pkgs.system}.inventree-cluster
                    }/bin/inventree-cluster
                  '';
                };
              };
            });
